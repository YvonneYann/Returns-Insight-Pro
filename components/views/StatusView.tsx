...
import { getSafeArray, getSafeObject, cleanReviewText, formatPercent, formatNumber } from '../../utils/formatters';

export const StatusView: React.FC<{ data: AppData }> = ({ data }) => {
                          ...
                          {reasonData.core_reasons.slice(0, 5).map((tag: any) => (
                            <div key={tag.tag_code}>
                              <div className="flex justify-between items-center text-xs mb-3">
                                <span className={`font-medium truncate pr-2 py-0.5 leading-5 ${tag.is_primary ? 'text-slate-800' : 'text-slate-500'}`}>
                                  {tag.tag_name_cn}
                                </span>
